...
trait GetBooksTrait
{
    ...
    {
        $books = $db->query(file_get_contents(Constants::SQL_SCRIPTS_PATH . '/get_books.sql'))->fetch_all(MYSQLI_ASSOC);
    ...
    public function findBookAuthors(Connection $db, $bookId, array &$book): void
    {
        $authorsIdentifiers = $db->query(
            file_get_contents(Constants::SQL_SCRIPTS_PATH . '/get_book_authors.sql'),
            ['i', [$bookId]]
        )->fetch_all();

        foreach ($authorsIdentifiers as $id) {
            $book['authors'][] = $db->query(
                file_get_contents(Constants::SQL_SCRIPTS_PATH . '/find_author_name.sql'),
                ['i', [$id[0]]]
            )->fetch_all()[0][0];
        }
    }
